...
def train_bpe(
    ...
    merges = []
    token_id = len(vocab)
    best_pairs = {"max_pairs_sorted": [], "good_pairs": set(), "resorting_value": 0, "resort_needed": True}

    for _ in range(num_merges):
        candidates, best_pairs = find_best_pair(
            candidates, vocab, best_pairs
        )  # sorted(candidates.items(), key=lambda x: (x[1], (vocab[x[0][0]], vocab[x[0][1]])), reverse=True)[0][0]
        ...
        counts, candidates, used_words, best_pairs = update_dictionaries(
            counts=counts, candidates=candidates, used_words=used_words, bps=best_pairs, token_id=token_id
        )
        token_id += 1
...
def find_best_pair(
    candidates: Counter[tuple[int, int] : int], vocab, bps: dict[list[tuple[int, int]], list[tuple[int, int]], int, int]
):
    ...
    if not bps["good_pairs"] or sort_needed:
        ...
        candidates = Counter(
            {pair: candidates[pair] for pair, _ in sorted_list}
        )  # Dubious. Updating for marginally faster sorting next time
        # print(candidates)
...
def update_dictionaries(
    counts: Counter[int], candidates: Counter[int], used_words: dict[list], bps: dict, token_id: int
):
    """
    ...
    for word_tuple in used_words[best_pair]:
        ...
        skip = False
        last_pair = None
        # need to merge in new token without screwing with existing structure.
        for c1, c2 in zip(word_tuple, word_tuple[1:]):
            if skip:
                skip = False
...
if __name__ == "__main__":
    ...
    with cProfile.Profile() as profile:
        ...
        vocab, merges = train_bpe(
            ...
            special_tokens=["<|endoftext|>", "<|imstart|>"],
            num_processes=8,
        )  # TinyStoriesV2-GPT4-valid.txt", vocab_size=270, special_tokens=[])
