...
def resource_accounting(config):
    ...
    embedding_params = (
        vocab_size * d_model
    )  # covers both embedding and final lm_head. Only one matrix due to weight sharing for gpt-2
    ...
    flops_per_part = {
        "Pos flops": pos_flops / total_flops,
...
if __name__ == "__main__":
    ...
    gpt2l_cfg = ResourceConfig(
        vocab_size=50257, context_length=1024, num_layers=36, d_model=1280, num_heads=20, d_ff=1280 * 4
    )
    gpt2m_cfg = ResourceConfig(
        vocab_size=50257, context_length=1024, num_layers=24, d_model=1024, num_heads=16, d_ff=1024 * 4
    )
    gpt2s_cfg = ResourceConfig(
        vocab_size=50257, context_length=1024, num_layers=12, d_model=768, num_heads=12, d_ff=768 * 4
    )

    # increased context length
    gpt2xlxc_cfg = ResourceConfig(
